...
수임계약 체결 **이전**의 잠재 의뢰인. **소유는 워크스페이스(회사)**. 상담원은 **볼 수 있는 권한·기본 뷰**를 부여받는 담당자.

| 필드 | 설명 |
|---|---|
| `id`, `workspace_id` | 기본 (소유자=워크스페이스) |
| `name`, `contact`, `contact_secondary` | 연락처 |
| `source` | 유입 채널 (`phone` / `kakao_ads` / `blog` / `referral` / `walk_in` / `other`) |
| `assigned_consultant_id` | 담당 상담원 (권한·뷰용 — 소유 아님) |
...
### 3.3 Payment (결제) & PaymentSchedule

Case 1개에 **계약서 1건** + **분할 회차 N건** 모델로 분리.

**3.3.1 PaymentContract** (계약서 = 1건)

한 Case에 대한 수임료 계약 **전체 윤곽**. 개인회생에서 "몇 분할 / 언제까지 / 얼마" 정보는 여기.

| 필드 | 설명 |
|---|---|
| `id`, `workspace_id`, `case_id` | 기본 |
| `total_amount_krw` | 계약 총액 (착수 + 중도 + 성공 등 합) |
| `plan_type` | `lump_sum`(일시) / `installment`(분할) / `conditional`(성공조건부) |
| `installment_count` | 분할 회차 수 (lump_sum이면 1) |
| `first_due_date`, `cycle_days` | 첫 납입일 + 주기 (예: 30일) |
| `payment_gate` | `hard`(결제 안 되면 업무 진행 불가) / `soft`(경고만) — 개인회생은 `hard` 기본 |
| `auto_dunning_enabled` | 자동 독촉 ON/OFF |
| `dunning_template_ids` | 발송 메시지 템플릿 (카톡·문자) |
| `dunning_schedule_days` | 연체 후 독촉 발송일 간격 (예: `[1, 3, 7, 14]` = D+1·D+3·D+7·D+14) |
| `signed_at`, `signed_by_user_id` | 계약 서명 |
| `cancelled_at`, `cancel_reason` | 해지 |
| `notes` | |

**3.3.2 PaymentSchedule** (회차 = N건)

계약의 각 분할 납입 건.

| 필드 | 설명 |
|---|---|
| `id`, `workspace_id`, `case_id`, `contract_id` | 기본 |
| `installment_no` | 1~N |
| `kind` | `retainer`(착수) / `installment`(분할) / `success_fee` / `court_fee` / `misc` |
| `amount_krw` | 이 회차 금액 |
| `paid_amount_krw` | 누적 수령액 (부분지급 허용) |
| `due_date` | 예정일 |
| `paid_date` | 완납 시각 |
| `status` | `scheduled` / `partial` / `paid` / `overdue` / `waived` / `refunded` |
| `dunning_count` | 독촉 발송 횟수 |
| `last_dunning_at` | 최근 독촉 시각 |
| `next_dunning_at` | 다음 예약 독촉 시각 (auto) |
| `payment_method` | `bank_transfer` / `card` / `cash` / `check` |
| `invoice_issued` | 세금계산서 발행 여부 |
| `gate_blocks_stages` | 이 회차 미납 시 차단할 Stage 키 배열 (예: `[filing, opening_decision]`) |

**상태 전이**: `scheduled → partial → paid` 또는 `scheduled → overdue`(due_date 지남).

**자동 독촉 규칙**:
1. 일 단위 cron이 `due_date < today AND status != paid` 스캔 → `status=overdue` 전이
2. `auto_dunning_enabled=true` + `next_dunning_at <= now()`면 메시지 발송 + Communication 생성 + `dunning_count++`
3. `dunning_schedule_days` 따라 다음 예약 계산

**Stage Gate (개인회생 특성)**:
- `payment_gate=hard`: 미납 회차 존재 + `gate_blocks_stages` 포함 Stage 진입 시도 → 차단
- 예: 착수금(installment_no=1) 미납 상태에서 `filing` Stage 진입 시도 → 거부
- Gate 해제는 납입 확인 즉시 (`confirm_payment` action)
...
## 7A. Pipeline View — 전사 현황 조망

역할 대시보드와 별도로, **"지금 사무실 전체에서 어떤 사건이 어느 Stage에 몇 건 있는지"** 를 한 화면에 보여주는 뷰.
...
병목 경고 (자동):
  🔴 보정 3차 2건 — 법원 마찰 장기화 (평균 18일)
  ⚠ 서류준비 8건 중 5건 14일 초과 — 의뢰인 응답 지연

미수금 경고 (결제팀 공동):
  💰 연체 5건 · 총 850만원 · 최장 21일 연체
```
...
| 요소 | 데이터 소스 |
|---|---|
| 컬럼별 카운트 | `cases` JOIN `stage_history (latest)` GROUP BY `stage_key` |
| 평균 체류일 | `stage_history.entry_date` 기준 현재 Stage 체류 |
| 병목 경고 | `stage_definitions.typical_duration_days` 초과 Case 개수 |
| 보정 N차 | `rehab_interactions.iteration_number` 최대값 |
| 미수금 | `payment_schedules WHERE status=overdue` |

### 7A.3 접근 권한

- **대표변호사**: 전체 도메인·전체 Stage
- **팀장**(향후 역할): 자기 팀 Stage만
- **개별 실무자**: 접근 불가 (대신 역할 대시보드로 충분)

### 7A.4 상호작용

- 컬럼 카운트 클릭 → 해당 Stage에 있는 Case 리스트 drill-down
- 병목 경고 클릭 → 원인 Case + 권장 Action 표시
- 카드 클릭 → Case 상세(`/workflow?case=<id>`)로 이동

### 7A.5 구현 메모

- `dashboard_configs`의 특수 타입 `pipeline_view`로 선언
- 범용 렌더러가 `object_type='case'` + `group_by='stage_key'`로 집계 쿼리 생성
- 실시간 필요 X. 5분 캐시 or revalidate 충분.
